...
export const ClaimsFilterHub: React.FC<ClaimsFilterHubProps> = ({
    ...
    return (
        <div className="bg-white dark:bg-[#16161a] apple-liquid-glass rounded-[24px] sm:rounded-[30px] md:rounded-[36px] p-2.5 sm:p-4 md:p-5 mb-3 sm:mb-4 md:mb-6 space-y-2.5 sm:space-y-3.5">
            {/* ==========================================================================
                1. PRIMARY: THE BIG TEAM COCKPIT CARDS (กระชับ หรูหรา ไม่เทอะทะบนมือถือ)
               ========================================================================== */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
                ...
                <button
                    ...
                    className={`relative p-2.5 sm:p-4 md:p-5 rounded-[18px] sm:rounded-[22px] md:rounded-[26px] text-left transition-all duration-200 flex flex-col justify-between overflow-hidden border cursor-pointer ${
                        teamFilter === 'ALL'
                ...
                >
                    <div className="flex items-center justify-between gap-1.5 sm:gap-2 mb-1.5 sm:mb-2.5">
                        <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
                            <div className={`w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-[10px] sm:rounded-[12px] flex items-center justify-center shrink-0 transition-transform ${
                            ...
                            <div className="min-w-0">
                                ...
                                <span className="text-[9.5px] sm:text-[10.5px] text-gray-400 dark:text-gray-500 truncate block">
                                    รวมทุกแบรนด์
                                </span>
                            </div>
                        </div>
                        <span className={`text-[8.5px] sm:text-[9.5px] md:text-[10.5px] font-black px-1.5 sm:px-2 py-0.5 rounded-full shrink-0 ${
                    ...
                    <div className="my-1 sm:my-1.5">
                        <div className="flex items-baseline gap-1.5 sm:gap-2">
                            <span className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black text-[#1d1d1f] dark:text-white tracking-tight">
                    ...
                    <div className="mt-1.5 pt-1.5 sm:mt-2 sm:pt-2 border-t border-black/5 dark:border-white/5 space-y-1">
                        <div className="flex items-center justify-between text-[9.5px] sm:text-[11px] text-gray-500 dark:text-gray-400">
                        ...
                        <div className="w-full h-1 sm:h-1.5 bg-gray-100 dark:bg-white/[0.08] rounded-full overflow-hidden">
                            <div 
                    ...
                    <div className="flex items-center justify-between gap-1.5 sm:gap-2 mb-1.5 sm:mb-2.5">
                        <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
                            <div className={`w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-[10px] sm:rounded-[12px] flex items-center justify-center shrink-0 transition-transform ${
                                ...
                                <span className="font-black text-xs sm:text-sm">B</span>
                            </div>
                            <div className="min-w-0">
                                <h3 className={`font-extrabold text-xs sm:text-sm md:text-base leading-tight truncate ${
                    ...
                    <div className="flex items-center justify-between gap-1.5 sm:gap-2 mb-1.5 sm:mb-2.5">
                        <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
                            <div className={`w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 rounded-[10px] sm:rounded-[12px] flex items-center justify-center shrink-0 transition-transform ${
                                ...
                                <span className="font-black text-xs sm:text-sm">C</span>
                            </div>
                            <div className="min-w-0">
                                <h3 className={`font-extrabold text-xs sm:text-sm md:text-base leading-tight truncate flex items-center gap-1 ${
                                    isTeamCExpanded || teamFilter === 'GROUP_C' || teamFilter === Team.TEAM_C || teamFilter === Team.TEAM_E || teamFilter === Team.TEAM_G
                                        ? 'text-violet-600 dark:text-violet-400'
                                        : 'text-[#1d1d1f] dark:text-white'
                                }`}>
                                    ทีม C
                                    <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${isTeamCExpanded ? 'rotate-180' : ''}`} />
                                </h3>
                                <span className="text-[9.5px] sm:text-[10.5px] text-gray-400 dark:text-gray-500 truncate block">
                                    Network · UPS · Online
                                </span>
            ...
            {/* ==========================================================================
                2. STATUS PIPELINE: APPLE CAPSULE SEGMENTED TRACK (แถวเดียวเพรียวบาง หรูหรา ไร้รอยต่อ)
               ========================================================================== */}
            <div className="bg-gray-150/70 dark:bg-black/40 p-1 sm:p-1.5 rounded-full border border-gray-200/75 dark:border-white/[0.06] shadow-inner backdrop-blur-md">
                <div className="grid grid-cols-4 gap-1 sm:gap-1.5">
                    {/* 2.1 ทั้งหมด */}
                    <button
                        type="button"
                        onClick={() => setStatusFilter('ALL')}
                        className={`h-8 sm:h-9 md:h-10 px-1 sm:px-2.5 md:px-4 rounded-full flex items-center justify-center gap-1 sm:gap-1.5 transition-all duration-150 cursor-pointer select-none border min-w-0 ${
                            statusFilter === 'ALL'
                    ...
                    >
                        <Package className={`w-3.5 h-3.5 shrink-0 hidden min-[390px]:block transition-colors ${
                            statusFilter === 'ALL' ? 'text-[#0071e3] dark:text-blue-400' : 'text-gray-400'
                        }`} />
                        <span className="font-bold text-[11px] sm:text-xs md:text-sm truncate">ทั้งหมด</span>
                        <span className={`text-[9.5px] sm:text-[11px] font-black px-1.5 sm:px-2 py-0.2 sm:py-0.5 rounded-full shrink-0 transition-colors ${
                            statusFilter === 'ALL'
                    ...
                    >
                        <Clock className={`w-3.5 h-3.5 shrink-0 hidden min-[390px]:block transition-colors ${
                            statusFilter === 'PENDING' ? 'text-blue-500' : 'text-blue-400/60'
                        }`} />
                        <span className="font-bold text-[11px] sm:text-xs md:text-sm truncate">รอรับเรื่อง</span>
                        <span className={`text-[9.5px] sm:text-[11px] font-black px-1.5 sm:px-2 py-0.2 sm:py-0.5 rounded-full shrink-0 transition-colors ${
                            statusFilter === 'PENDING'
                    ...
                    >
                        <Wrench className={`w-3.5 h-3.5 shrink-0 hidden min-[390px]:block transition-colors ${
                            statusFilter === 'IN_PROGRESS' ? 'text-amber-500' : 'text-amber-400/60'
                        }`} />
                        <span className="font-bold text-[11px] sm:text-xs md:text-sm truncate">
                            <span className="hidden sm:inline">กำลัง</span>ดำเนินการ
                        </span>
                        <span className={`text-[9.5px] sm:text-[11px] font-black px-1.5 sm:px-2 py-0.2 sm:py-0.5 rounded-full shrink-0 transition-colors ${
                            statusFilter === 'IN_PROGRESS'
                    ...
                    >
                        <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 hidden min-[390px]:block transition-colors ${
                            statusFilter === 'DONE' ? 'text-emerald-500' : 'text-emerald-400/60'
                        }`} />
                        <span className="font-bold text-[11px] sm:text-xs md:text-sm truncate">
                            เสร็จสิ้น<span className="hidden sm:inline">แล้ว</span>
                        </span>
                        <span className={`text-[9.5px] sm:text-[11px] font-black px-1.5 sm:px-2 py-0.2 sm:py-0.5 rounded-full shrink-0 transition-colors ${
                            statusFilter === 'DONE'
