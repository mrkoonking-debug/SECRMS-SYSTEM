        ...
        <div className="max-w-6xl mx-auto px-3 md:px-6 pb-24 md:pb-8">
            ...
            <div className="mb-4 md:mb-6 space-y-2.5">
                <div className="grid grid-cols-4 md:flex gap-2 md:gap-2 pb-1">
                    <button onClick={() => { setTeamFilter('ALL'); setIsTeamCExpanded(false); }} className={`rounded-2xl md:rounded-[20px] px-2 py-2 md:px-5 md:py-4 text-left transition-all duration-200 md:flex-1 ${teamFilter === 'ALL' ? 'bg-blue-600 dark:bg-blue-600 shadow-sm border border-blue-500/30' : 'bg-white dark:bg-white/[0.06] border border-gray-200/80 dark:border-white/[0.08] shadow-sm hover:border-blue-300 dark:hover:border-blue-500/30 active:scale-[0.97]'}`}><div className={`text-[8px] md:text-[10px] font-bold uppercase tracking-wider mb-0.5 truncate ${teamFilter === 'ALL' ? 'text-blue-200' : 'text-gray-400 dark:text-gray-500'}`}>{t('claimsList.active')}</div><div className={`text-base md:text-2xl font-extrabold ${teamFilter === 'ALL' ? 'text-white' : 'text-[#1d1d1f] dark:text-white'}`}>{rmas.filter(c => c.status !== RMAStatus.CLOSED && c.status !== RMAStatus.CANCELLED).length}</div></button>
                    <button onClick={() => { setTeamFilter(Team.HIKVISION); setIsTeamCExpanded(false); }} className={`rounded-2xl md:rounded-[20px] px-2 py-2 md:px-5 md:py-4 text-left transition-all duration-200 md:flex-1 ${teamFilter === Team.HIKVISION ? 'bg-red-500 dark:bg-red-600 shadow-sm border border-red-400/30' : 'bg-white dark:bg-white/[0.06] border border-gray-200/80 dark:border-white/[0.08] shadow-sm hover:border-red-300 dark:hover:border-red-500/30 active:scale-[0.97]'}`}><div className={`text-[8px] md:text-[10px] font-bold uppercase tracking-wider mb-0.5 ${teamFilter === Team.HIKVISION ? 'text-red-200' : 'text-red-400 dark:text-red-400/60'}`}>HIK</div><div className={`text-base md:text-2xl font-extrabold ${teamFilter === Team.HIKVISION ? 'text-white' : 'text-[#1d1d1f] dark:text-white'}`}>{getTeamCount(Team.HIKVISION)}</div></button>
                    <button onClick={() => { setTeamFilter(Team.DAHUA); setIsTeamCExpanded(false); }} className={`rounded-2xl md:rounded-[20px] px-2 py-2 md:px-5 md:py-4 text-left transition-all duration-200 md:flex-1 ${teamFilter === Team.DAHUA ? 'bg-orange-500 dark:bg-orange-600 shadow-sm border border-orange-400/30' : 'bg-white dark:bg-white/[0.06] border border-gray-200/80 dark:border-white/[0.08] shadow-sm hover:border-orange-300 dark:hover:border-orange-500/30 active:scale-[0.97]'}`}><div className={`text-[8px] md:text-[10px] font-bold uppercase tracking-wider mb-0.5 ${teamFilter === Team.DAHUA ? 'text-orange-200' : 'text-orange-400 dark:text-orange-400/60'}`}>DAHUA</div><div className={`text-base md:text-2xl font-extrabold ${teamFilter === Team.DAHUA ? 'text-white' : 'text-[#1d1d1f] dark:text-white'}`}>{getTeamCount(Team.DAHUA)}</div></button>
                    <button onClick={handleGroupCClick} className={`rounded-2xl md:rounded-[20px] px-2 py-2 md:px-5 md:py-4 text-left transition-all duration-200 md:flex-1 ${isTeamCExpanded || teamFilter === 'GROUP_C' ? 'bg-violet-500 dark:bg-violet-600 shadow-sm border border-violet-400/30' : 'bg-white dark:bg-white/[0.06] border border-gray-200/80 dark:border-white/[0.08] shadow-sm hover:border-violet-300 dark:hover:border-violet-500/30 active:scale-[0.97]'}`}><div className={`text-[8px] md:text-[10px] font-bold uppercase tracking-wider mb-0.5 ${isTeamCExpanded || teamFilter === 'GROUP_C' ? 'text-violet-200' : 'text-violet-400 dark:text-violet-400/60'}`}>Team C</div><div className={`text-base md:text-2xl font-extrabold ${isTeamCExpanded || teamFilter === 'GROUP_C' ? 'text-white' : 'text-[#1d1d1f] dark:text-white'}`}>{getGroupCCount()}</div></button>
                </div>
                {isTeamCExpanded && (
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide animate-fade-in pl-2 border-l-2 border-violet-500/30">
                        <button onClick={() => setTeamFilter(Team.TEAM_C)} className={`rounded-xl px-4 py-2 border whitespace-nowrap text-sm transition-all ${teamFilter === Team.TEAM_C ? 'border-cyan-400 text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-500/10' : 'bg-gray-50 dark:bg-white/[0.04] border-gray-200/80 dark:border-white/[0.08]'} dark:text-white`}>Network ({getTeamCount(Team.TEAM_C)})</button>
                        <button onClick={() => setTeamFilter(Team.TEAM_E)} className={`rounded-xl px-4 py-2 border whitespace-nowrap text-sm transition-all ${teamFilter === Team.TEAM_E ? 'border-yellow-400 text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-500/10' : 'bg-gray-50 dark:bg-white/[0.04] border-gray-200/80 dark:border-white/[0.08]'} dark:text-white`}>UPS ({getTeamCount(Team.TEAM_E)})</button>
                        <button onClick={() => setTeamFilter(Team.TEAM_G)} className={`rounded-xl px-4 py-2 border whitespace-nowrap text-sm transition-all ${teamFilter === Team.TEAM_G ? 'border-fuchsia-400 text-fuchsia-600 dark:text-fuchsia-400 bg-fuchsia-50 dark:bg-fuchsia-500/10' : 'bg-gray-50 dark:bg-white/[0.04] border-gray-200/80 dark:border-white/[0.08]'} dark:text-white`}>Online ({getTeamCount(Team.TEAM_G)})</button>
                    </div>
                )}
            </div>

            {/* ── Search + Filters ── */}
            <div className="bg-white dark:bg-white/[0.05] rounded-xl md:rounded-2xl shadow-sm border border-gray-200/80 dark:border-white/[0.08] p-1.5 mb-5 md:mb-6 sticky top-14 md:top-0 z-30 flex flex-col md:flex-row md:items-center gap-1 md:gap-2">
                ...
                <div className="flex items-center gap-1 flex-wrap md:flex-nowrap px-1 md:px-0 pb-0.5 md:pb-0">
                    ...
                    <div className="hidden md:block h-5 w-px bg-gray-200 dark:bg-white/10 flex-shrink-0"></div>
                    {['ALL', 'PENDING', 'IN_PROGRESS', 'DONE'].map((s) => (
                        <button key={s} onClick={() => setStatusFilter(s as typeof statusFilter)} className={`flex-1 md:flex-none px-2 md:px-3.5 py-1.5 md:py-2 text-[10px] md:text-xs font-medium rounded-lg md:rounded-xl transition-all whitespace-nowrap text-center ${statusFilter === s ? 'bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] shadow-sm' : 'text-gray-500 hover:text-[#1d1d1f] dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/[0.06]'}`}>{s === 'ALL' ? t('claimsList.filterStatus') : t(`status.${s}`)}</button>
                    ))}
                    {isAnyFilterActive && (
                        <button onClick={handleClearFilters} className="flex-1 md:flex-none px-2 md:px-3.5 py-1.5 md:py-2 text-[10px] md:text-xs font-semibold rounded-lg md:rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 transition-all whitespace-nowrap flex items-center justify-center gap-1">
                            <X className="w-3 h-3 md:w-3.5 md:h-3.5" /> {t('claimsList.clearFilters')}
                        </button>
                    )}
                </div>
            </div>
                                ...
                                {isDateExpanded && (
                                    <div className="space-y-3 md:space-y-4 pl-3 md:pl-0">
                                        {sortedJobKeys.map(jobKey => {
                                            ...
                                            return (
                                                <div key={jobKey} onClick={() => handleJobClick(jobKey)} className={`bg-white dark:bg-white/[0.05] rounded-[20px] md:rounded-[24px] overflow-hidden border cursor-pointer transition-all duration-200 group will-change-transform shadow-sm ${isJobDone ? 'border-green-200/60 dark:border-green-500/10' : 'border-gray-200/80 dark:border-white/[0.08]'} hover:shadow-md hover:shadow-black/[0.04] dark:hover:shadow-black/30 hover:-translate-y-0.5 hover:border-blue-200 dark:hover:border-blue-500/20 active:scale-[0.99] active:translate-y-0`}>
