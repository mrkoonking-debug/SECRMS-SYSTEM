...
import { SEED_PETTY_CASH } from './pettyCashData';
...
let OFFLINE_PETTY_CASH: PettyCashTransaction[] = [...SEED_PETTY_CASH];
...
export const MockDb = {
  ...
  seedDatabase: async () => {
    ...
    try {
      ...
      // Seed Petty Cash Ledger (262 records in batches)
      await MockDb.seedPettyCashDatabase();
    ...
    try {
      ...
      for (let i = 0; i < SEED_PETTY_CASH.length; i += batchSize) {
        ...
        for (const tx of chunk) {
          const docRef = doc(db, 'pettycash', tx.id);
          const { id, ...data } = tx;
          batch.set(docRef, {
            ...data,
            isDeleted: false,
